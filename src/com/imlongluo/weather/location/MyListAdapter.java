 ...
 * @author longluo 
 * 自定义的可伸缩性列表的适配器
 *         例如下代码所示，使用时，A,B二级列表项将在一级列表组group1下： <code>
 ...
 * </code> 使用时注意各级列表间的对应关系
 */
public class MyListAdapter extends BaseExpandableListAdapter implements
		Filterable {
	...
	/**
	 * 构造方法，指定一级菜单项，二组菜单项，来构造一个可伸缩性列表的适配器 其中的childs要与groups的索引值对应 <br />
	 * 
	 * @param context
	 *            为 <code>Context</code>对象，用于记录此控件的上下文本对象
	 * @param groups
	 *            为<code>String</code>类型数组，定义的一级菜单项数组
	 * @param childs
	 *            为<code>String</code>类型数组,与groups索引值相对应的二级菜单项二维数组
	 */
	public MyListAdapter(Context context, ExpandableListView listView,
